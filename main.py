...
async def alice_webhook(request: Request):
    ...
    if is_new_session:
        ...
        response_text = "Функция календаря пока в разработке."
    elif any(word in user_text for word in ["почт", "письм", "email"]):
        response_text = "Функция почты пока в разработке."
    elif any(word in user_text for word in ["файл", "документ", "диск"]):
        response_text = "Функция поиска по диску пока в разработке."
    elif any(word in user_text for word in ["помощь", "помоги", "что умеешь"]):
        response_text = (
        ...
        return {
            "response": {"text": "До встречи!", "end_session": True},
            "version": "1.0",
        }
    else:
        response_text = f"Ты сказал: «{user_text}». Попробуй сказать: помощь."
